...
{	
	...
	{			  
		...
		if(dat&0x80)
		nrf_gpio_pin_set(cin);
		else 
		nrf_gpio_pin_clear(cin);
...
void OLED_Char_clear(uint8_t x,uint8_t y,uint8_t chr)
{      	
...
void OLED_String_clear(uint8_t x,uint8_t y,uint8_t *chr)
{
	unsigned char j=0;
	while (chr[j]!='\0')
	{		OLED_Char_clear(x,y,chr[j]);
			x+=8;
		if(x>120){x=0;y+=2;}
			j++;
	}
